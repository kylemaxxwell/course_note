...
**Full outer join**<BR>To keep all rows from both data frames, specify `all=TRUE`.<BR>`mergedData = merge(DF1,DF2,by.x="id1",by.y="id2",all=TRUE)`|**An outer join**<BR>`merge(DT1, DT2, all=TRUE)`|`library(plyr)`<BR>`join(x, y, by = NULL, type = "full")`<BR>`join_all(a_list, by = NULL, type = "full")`<BR><BR>`library(dplyr)`<BR>`full_join(x, y)`
